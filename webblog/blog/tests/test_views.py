...
class BlogListViewTest(TestCase):
    ...
    def test_pagination_is_ten(self):
        ...
        self.assertEqual(len(response.context["blog_list"]), expected_pagination_number)
...
class BlogDetailCommentViewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.test_user.picture.save(
            "%s.png" % (cls.test_user.email[0] + str(cls.test_user.pk)), ContentFile(s)
        )
        cls.test_user.is_active = True
        cls.test_user.save()
    ...
    def test_view_get_correct_blog_detail(self):
        ...
        self.assertEqual(
            response.context_data["blog"].description, self.blog_description
        )
...
class BlogCreateViewTest(TestCase):
    ...
    def test_add_new_blog_post_success(self):
        data = {"title": self.blog_title, "description": self.blog_description}
        self.client.login(email=self.user_email, password=self.user_password)
...
class BlogDeleteViewTest(TestCase):
    ...
    def test_view_get_desired_success_url(self):
        ...
        self.client.login(email=self.user_email, password=self.user_password)
        response = self.client.post(reverse("blog-delete", kwargs={"pk": self.blog.pk}))
    ...
    def setUpTestData(cls):
        ...
        for blog_num in range(number_of_blogs):
            ...
            blog = Blog.objects.create(
                title=title, author=cls.test_user, description=description
            )
