...
class MyUserModelsTests(TestCase):
    ...
    def test_email_correct_label(self):
        ...
        self.assertEqual(
            self.test_user._meta.get_field("email").verbose_name, expected_email_label
        )

    def test_name_correct_label(self):
        expected_name_label = "username"
        self.assertEqual(
            self.test_user._meta.get_field("name").verbose_name, expected_name_label
        )

    def test_bio_correct_label(self):
        expected_bio_label = "bio"
        self.assertEqual(
            self.test_user._meta.get_field("bio").verbose_name, expected_bio_label
        )

    def test_date_of_birth_correct_label(self):
        expected_date_of_birth_label = "birthdate"
        self.assertEqual(
            self.test_user._meta.get_field("date_of_birth").verbose_name,
            expected_date_of_birth_label,
        )

    def test_picture_correct_labe(self):
        expected_picture_label = "profile picture"
        self.assertEqual(
            self.test_user._meta.get_field("picture").verbose_name,
            expected_picture_label,
        )
    ...
    def test_picture_save_with_wrong_size(self):
        self.test_user.picture = self.get_image_file("image.jpg", "png", (500, 500))
        self.test_user.save()
        image = Image.open(self.test_user.picture.path)
        actual_image_width, actual_image_height = image.size
        self.assertLessEqual(actual_image_width, self.image_width)
