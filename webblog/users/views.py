...
class BloggerDashboardView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(BloggerDashboardView, self).get_context_data(
            *args, **kwargs)
        blogs = Blog.objects.filter(author=self.request.user)
        total_blog_comments = blogs.annotate(total_comments=Count('blogcomment')).aggregate(
            total_comments_sum=Sum('total_comments')).get('total_comments_sum', 0)
